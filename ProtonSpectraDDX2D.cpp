...
void ProtonSpectraDDX2D()
{

	TGraph2D *Graph = new TGraph2D();
	loadDdxSpectra2D ("ddx/d+U235.o.ddxmode3", Ejectile::Proton, Graph);

	TString name = "Proton_differential_cross_section_for_U235";
	auto c1 = new TCanvas(name, name,1100,800);

	Graph -> Draw("surf2");
	Graph -> SetLineColor(kMagenta);
	Graph -> SetTitle("Proton differential cross section for U235;Energy (MeV);Angle;Cross Section");
